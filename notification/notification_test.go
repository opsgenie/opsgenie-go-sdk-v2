...
func TestCreateRuleRequest_Validate(t *testing.T) {
	...
	err = createRequest.Validate()
	assert.Equal(t, err.Error(), errors.New("Type of schedule cannot be empty.").Error())
	...
	err = createRequest.Validate()
	assert.Equal(t, err.Error(), errors.New("Type of criteria cannot be empty.").Error())

	createRequest.Criteria = &og.Criteria{CriteriaType: og.MatchAnyCondition}
	err = createRequest.Validate()
	assert.Equal(t, err.Error(), errors.New("Conditions cannot be empty.").Error())
